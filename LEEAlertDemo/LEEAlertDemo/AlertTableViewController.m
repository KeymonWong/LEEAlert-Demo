...
- (void)demo:(NSInteger)index{
    ...
    switch (index) {
        ...
        {
            [LEEAlert alert].config
            .LeeShouldShowCloseBtn(YES)
            .LeeImage([UIImage imageNamed:@"icon_prompt"])
            .LeeTitle(@"评个分吧")
            .LeeAddContent(^(UILabel *label) {
                
//                label.text = @"如果您觉得不错, 那就给个五星好评吧 亲~";
                
                NSString *txt = @"如果您觉得不错, 那就给个五星好评吧亲~";
                
                label.textColor = [UIColor grayColor];
                
                NSMutableAttributedString *muAttr = [[NSMutableAttributedString alloc] initWithString:txt];
                [muAttr addAttribute:NSForegroundColorAttributeName value:[UIColor blackColor] range:NSMakeRange(txt.length-8, 8)];
                [muAttr addAttribute:NSUnderlineStyleAttributeName value:[NSNumber numberWithInteger:NSUnderlineStylePatternSolid] range:NSMakeRange(0, 10)];
                [muAttr addAttribute:NSUnderlineColorAttributeName value:[UIColor greenColor] range:NSMakeRange(0, 10)];
                [muAttr addAttribute:NSFontAttributeName value:[UIFont boldSystemFontOfSize:17.] range:NSMakeRange(txt.length-4, 4)];
...
//            })
            .LeeAddAction(^(LEEAction *action) {
                ...
                action.insets = UIEdgeInsetsMake(0, 40, 20, 40);
